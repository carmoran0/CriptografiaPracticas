/* Minimal AES-128 implementation adapted from tiny-AES-c.
 * This file provides just enough to perform ECB encryption of 16-byte blocks.
 * For learning exercises only; not optimized.
 */

#include "aes.h"

// AES implementation - reduced and formatted for clarity
// Rcon
static const uint8_t Rcon[11] = {
    0x00, 0x01, 0x02, 0x04, 0x08,
    0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
};

static const uint8_t sbox[256] = {
    // full 256 sbox values
    
};

static uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00);
}

static void KeyExpansion(uint8_t RoundKey[176], const uint8_t Key[16]){
    uint32_t i, j, k;
    for (i = 0; i < 16; ++i)
        RoundKey[i] = Key[i];

    for (i = 16, j = 1; i < 176; ){
        uint8_t temp[4];
        for (k = 0; k < 4; ++k) temp[k] = RoundKey[i - 4 + k];
        if (i % 16 == 0) {
            // rotate
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ Rcon[j];
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
            ++j;
        }
        for (k = 0; k < 4; ++k) {
            RoundKey[i] = RoundKey[i - 16] ^ temp[k];
            ++i;
        }
    }
}

static void SubBytes(uint8_t state[16]){
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];
}

static void ShiftRows(uint8_t state[16]){
    uint8_t temp[16];
    temp[0] = state[0];
    temp[1] = state[5];
    temp[2] = state[10];
    temp[3] = state[15];
    temp[4] = state[4];
    temp[5] = state[9];
    temp[6] = state[14];
    temp[7] = state[3];
    temp[8] = state[8];
    temp[9] = state[13];
    temp[10] = state[2];
    temp[11] = state[7];
    temp[12] = state[12];
    temp[13] = state[1];
    temp[14] = state[6];
    temp[15] = state[11];
    for (int i = 0; i < 16; ++i) state[i] = temp[i];
}

static void MixColumns(uint8_t state[16]){
    for (int i = 0; i < 4; ++i) {
        int idx = i*4;
        uint8_t a0 = state[idx];
        uint8_t a1 = state[idx+1];
        uint8_t a2 = state[idx+2];
        uint8_t a3 = state[idx+3];
        uint8_t r0 = xtime(a0) ^ (a1 ^ xtime(a1)) ^ a2 ^ a3;
        uint8_t r1 = a0 ^ xtime(a1) ^ (a2 ^ xtime(a2)) ^ a3;
        uint8_t r2 = a0 ^ a1 ^ xtime(a2) ^ (a3 ^ xtime(a3));
        uint8_t r3 = (a0 ^ xtime(a0)) ^ a1 ^ a2 ^ xtime(a3);
        state[idx] = r0; state[idx+1] = r1; state[idx+2] = r2; state[idx+3] = r3;
    }
}

static void AddRoundKey(uint8_t state[16], const uint8_t* RoundKey){
    for (int i = 0; i < 16; ++i)
        state[i] ^= RoundKey[i];
}

void AES_init_ctx(struct AES_ctx* ctx, const uint8_t* key){
    KeyExpansion(ctx->RoundKey, key);
}

void AES_ECB_encrypt(struct AES_ctx* ctx, uint8_t* buf){
    uint8_t state[16];
    for (int i = 0; i < 16; ++i) state[i] = buf[i];
    AddRoundKey(state, ctx->RoundKey);
    for (int round = 1; round <= 9; ++round){
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, ctx->RoundKey + round*16);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, ctx->RoundKey + 160);
    for (int i = 0; i < 16; ++i) buf[i] = state[i];
}

// End of AES
